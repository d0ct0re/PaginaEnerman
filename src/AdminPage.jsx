  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 24 }}>
        {[
          { label: "Clientes registrados", valor: clientes.length, color: "#e0b11e" },
          { label: "Este mes", valor: clientes.filter(c => {
            if (!c.fechaRegistro?.toDate) return false;
            const d = c.fechaRegistro.toDate();
            const now = new Date();
            return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
          }).length, color: "#22c55e" },
          { label: "Activos", valor: clientes.filter(c => c.activo !== false).length, color: "#3b82f6" },
        ].map(s => (
      ...
      <input
        ...
        style={{ width:"100%", background:"#0e0e0e", border:"1px solid #1a1a1a", borderRadius:10,
          padding:"10px 16px", color:"#fff", fontSize:14, marginBottom:16, boxSizing:"border-box" }}
      />

      {/* Tabla */}
      {cargando ? (
        <div style={{ textAlign:"center", color:"#555", padding:40 }}>Cargando clientes…</div>
      ) : filtrados.length === 0 ? (
        <div style={{ textAlign:"center", color:"#555", padding:40 }}>
          {busqueda ? "Sin resultados." : "Aún no hay clientes registrados."}
        </div>
      ) : (
        <div style={{ background:"#0a0a0a", borderRadius:14, border:"1px solid #1a1a1a", overflow:"hidden" }}>
          {/* Header */}
          <div style={{ display:"grid", gridTemplateColumns:"1fr 160px 140px 80px",
            padding:"10px 20px", borderBottom:"1px solid #1a1a1a",
            fontSize:11, color:"#555", fontWeight:700, letterSpacing:1, textTransform:"uppercase" }}>
            <span>Nombre</span><span>Teléfono</span><span>Registro</span><span>Estado</span>
          </div>
          {filtrados.map((c, i) => (
            <div key={c.uid} style={{
              display:"grid", gridTemplateColumns:"1fr 160px 140px 80px",
              padding:"14px 20px", borderBottom: i < filtrados.length-1 ? "1px solid #111" : "none",
              alignItems:"center", fontSize:14,
            }}>
              <span style={{ color:"#e8e8e8", fontWeight:600 }}>{c.nombre || <em style={{color:"#555"}}>Sin nombre</em>}</span>
              <span style={{ color:"#888", fontFamily:"monospace" }}>{c.telefono || "—"}</span>
              <span style={{ color:"#666", fontSize:12 }}>{fmtFecha(c.fechaRegistro)}</span>
              <span style={{
                background: c.activo !== false ? "#14532d" : "#450a0a",
                color: c.activo !== false ? "#4ade80" : "#f87171",
                fontSize:11, padding:"2px 8px", borderRadius:20, fontWeight:600, width:"fit-content"
              }}>
...
function AdminPage() {
  ...
  const stats = {
    ...
    conVariantes: productos.filter(p => p.tiene_variantes && p.variantes?.length > 0).length,
    ...
    <div style={{ minHeight: "100vh", background: "#050505", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#0a0a0a", borderBottom: "1px solid #1a1a1a", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <button
            onClick={() => logout()}
            style={{ background: "transparent", color: "#888", border: "1px solid #333", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 12 }}
          >
            Cerrar sesión
          </button>
          {tab === "productos" && (
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: "Con variantes", valor: stats.conVariantes, color: "#3b82f6" },
      ...
      {vistaDetalle && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
          ...
          <div style={{ background: "#0e0e0e", borderRadius: 20, padding: 32, maxWidth: 700, width: "100%", maxHeight: "85vh", overflowY: "auto", border: "1px solid #222" }}
            ...
            {[vistaDetalle.imagen_principal, vistaDetalle.imagen_2, vistaDetalle.imagen_3].filter(Boolean).length > 1 && (
              <div>
                <p style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Imágenes adicionales</p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {[vistaDetalle.imagen_principal, vistaDetalle.imagen_2, vistaDetalle.imagen_3].filter(Boolean).map((img, i) => (
...
function ModalEditar({ producto, modoNuevo, guardando, onGuardar, onCerrar }) {
  // Normaliza variantes: acepta tanto "nombre" como "nommbre" (typo existente en BD)
  const normalizarVariantes = (vars) =>
    (vars || []).map(v => ({
      id:         v.id || "",
      nombre:     v.nombre || v.nommbre || "",
      precio:     Number(v.Precio ?? v.precio ?? 0),
  ...
  // Cuando se activan variantes y hay precios, auto-calcula precio_desde
  const precioMinVariantes = variantes.length > 0
    ? Math.min(...variantes.filter(v => v.precio > 0).map(v => v.precio))
    : 0;

  const agregarVariante = () => {
    setVariantes(prev => [...prev, { id: "", nombre: "", precio: 0, disponible: true }]);
  };
  ...
  const handleGuardar = () => {
    ...
    const precioFinal = form.tiene_variantes && precioMinVariantes > 0
      ? precioMinVariantes
      : Number(form.precio_desde) || 0;

    const datos = {
      ...form,
      precio_desde:    precioFinal,
      imagenes:        [form.imagen_principal, form.imagen_2, form.imagen_3].filter(Boolean),
      search_terms:    [form.categoria, form.marca, form.subcategoria, form.title].map(s => s.toLowerCase()),
    };
  ...
  const inputStyle = { width: "100%", background: "#151515", border: "1px solid #222", borderRadius: 10, padding: "10px 14px", color: "#fff", fontSize: 13, outline: "none", boxSizing: "border-box" };
  const labelStyle = { display: "block", fontSize: 11, color: "#555", marginBottom: 6, textTransform: "uppercase", letterSpacing: 1 };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
      onClick={onCerrar}>
      <div style={{ background: "#0e0e0e", borderRadius: 20, padding: 32, maxWidth: 680, width: "100%", maxHeight: "92vh", overflowY: "auto", border: "1px solid #222" }}
        onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div>
            {/* Precio manual — se desactiva si tiene variantes */}
            <div style={{ marginBottom: 16 }}>
              <label style={labelStyle}>Precio MXN (0 = consultar)</label>
              <input
                type="number"
                value={form.tiene_variantes ? precioMinVariantes : form.precio_desde}
                onChange={e => set("precio_desde", Number(e.target.value))}
                disabled={form.tiene_variantes}
                style={{ ...inputStyle, opacity: form.tiene_variantes ? 0.4 : 1, cursor: form.tiene_variantes ? "not-allowed" : "text" }}
              />
              {form.tiene_variantes && precioMinVariantes > 0 && (
                <p style={{ fontSize: 11, color: "#4ade80", margin: "4px 0 0" }}>
                  Auto: mínimo de variantes = ${precioMinVariantes}
                </p>
              )}
            </div>
          </div>
        ...
        {/* Imágenes */}
        <p style={{ ...labelStyle, marginBottom: 10 }}>Imágenes (URLs)</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
          {[["imagen_principal", "Principal"], ["imagen_2", "Imagen 2"], ["imagen_3", "Imagen 3"]].map(([key, lbl]) => (
            <div key={key}>
              <label style={{ ...labelStyle, marginBottom: 4 }}>{lbl}</label>
              <input value={form[key]} onChange={e => set(key, e.target.value)}
                placeholder="https://..." style={inputStyle} />
              {form[key] && (
                <img src={form[key]} alt="" style={{ width: "100%", height: 70, objectFit: "cover", borderRadius: 8, marginTop: 6, background: "#1a1a1a" }}
        ...
        <div style={{ display: "flex", gap: 24, marginBottom: 24 }}>
          {[
            ...
            <div key={id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="checkbox" id={id} checked={form[key]} onChange={e => set(key, e.target.checked)} />
              <label htmlFor={id} style={{ fontSize: 13, color: "#888", cursor: "pointer" }}>{label}</label>
            </div>
          ))}
        </div>
        ...
        {form.tiene_variantes && (
          <div style={{ border: "1px solid #2a2a2a", borderRadius: 14, padding: 20, marginBottom: 24 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#e0b11e" }}>
                Variantes ({variantes.length})
              </p>
              <button onClick={agregarVariante}
                style={{ background: "#e0b11e", color: "#000", border: "none", borderRadius: 8, padding: "6px 14px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>
                + Agregar variante
              </button>
            </div>
            ...
            {variantes.length === 0 && (
              <p style={{ color: "#444", fontSize: 13, textAlign: "center", padding: "12px 0" }}>
                Sin variantes. Haz clic en "+ Agregar variante".
              </p>
            )}

            {/* Cabecera de columnas */}
            {variantes.length > 0 && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr 100px 80px 36px", gap: 8, marginBottom: 8 }}>
                {["ID / SKU", "Nombre / Medida", "Precio MXN", "Disponible", ""].map(h => (
                  <span key={h} style={{ fontSize: 10, color: "#444", textTransform: "uppercase", letterSpacing: 1 }}>{h}</span>
                ))}
              </div>
            )}

            {variantes.map((v, idx) => (
              <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr 100px 80px 36px", gap: 8, marginBottom: 8, alignItems: "center" }}>
                <input
                  value={v.id}
                  onChange={e => actualizarVariante(idx, "id", e.target.value)}
                  placeholder="TCPD13RY"
                ...
                <input
                  type="number"
                  value={v.precio}
                  onChange={e => actualizarVariante(idx, "precio", Number(e.target.value))}
                  placeholder="0"
                  style={{ ...inputStyle, padding: "8px 10px", fontSize: 12 }}
                />
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <input
                    ...
                    onChange={e => actualizarVariante(idx, "disponible", e.target.checked)}
                  />
                </div>
                <button onClick={() => eliminarVariante(idx)}
                  style={{ background: "#2a1a1a", color: "#f87171", border: "none", borderRadius: 8, width: 36, height: 36, cursor: "pointer", fontSize: 16 }}>
                  ✕
                </button>
              </div>
            ))}
